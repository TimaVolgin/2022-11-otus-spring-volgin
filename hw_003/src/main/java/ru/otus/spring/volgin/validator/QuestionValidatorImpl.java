    ...
    public void validate(List<Question> questionList) {
        questionList.forEach(question -> {
            if (question.getAnswers() == null || question.getAnswers().size() < 2) {
                throw new IllegalStateException(format("У вопроса \"{0}\" должно быть несколько вариантов ответа", question.getText()));
            }
            if (question.getAnswers().size() - 1 < question.getCorrectAnswerIndex() || question.getCorrectAnswerIndex() < 0) {
                throw new IllegalStateException(format("У вопроса \"{0}\" указан неверный индекс ответа", question.getText()));
            }
        });
    }
}
