...
public class BookServiceImpl implements BookService {
    ...
    public BookWithCommentsDto findById(long id) {
        return bookMapper.toDtoWithComments(bookRepository.findById(id)
                .orElseThrow(() -> new NotFoundException(format("Не найдена книга с идентификатором {0}", id))));
    }
    ...
    private Genre getGenre(long genreId) {
        return genreRepository.findById(genreId)
                .orElseThrow(() -> new NotFoundException(format("Не найден жанр с именем {0}", genreId)));
    }

    private Author getAuthor(long authorId) {
        return authorRepository.findById(authorId)
                .orElseThrow(() -> new NotFoundException(format("Не найден автор с идентификатором {0}", authorId)));
    }
}
