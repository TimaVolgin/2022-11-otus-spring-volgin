...
public class GenreDaoJdbc implements GenreDao {
    ...
    public Optional<Genre> findByName(String name) {
        return Optional.ofNullable(jdbc.query(SELECT_GENRE_BY_NAME, Map.of("name", name),
                rs -> rs.next() ? genreMapper.mapRow(rs, 1) : null));
    }

    @Override
    public Optional<Genre> findById(long id) {
        return Optional.ofNullable(jdbc.query(SELECT_GENRE_BY_ID, Map.of("id", id),
                rs -> rs.next() ? genreMapper.mapRow(rs, 1) : null));
    }
