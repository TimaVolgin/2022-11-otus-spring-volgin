...
public class RestExceptionHandler {
    ...
    public final ErrorInfo handleBindException(BindException ex, WebRequest request) {
        var result = ex.getAllErrors().stream().map(DefaultMessageSourceResolvable::getDefaultMessage).collect(Collectors.joining(". "));
        return new ErrorInfo(HttpStatus.UNPROCESSABLE_ENTITY.getReasonPhrase(), result, request.getDescription(false));
    }
}
