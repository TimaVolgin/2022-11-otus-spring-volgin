...
public class CommentCommands {
    ...
    @ShellMethod(key = {"fcbi", "findCommentsByBookId"},
            value = "Ищет комментарии по идентификатору книги. Пример: findCommentsByBookId 1")
    public String findCommentsByBookId(@ShellOption(value = {"id"}, help = "Идентификатор книги") String id) {
        return format("Список комментариев:\n\t{0}",
                commentService.findByBookId(id).stream()
                        .map(Comment::toString)
                        .collect(Collectors.joining("\n\t")));
    }
